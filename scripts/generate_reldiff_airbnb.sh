#!/usr/bin/env bash
# Generate RelDiff synthetic data for airbnb — 3 seeds in parallel, one per GPU.
# Run from MT-MIA/ root:  bash scripts/generate_reldiff_airbnb.sh
...
train_and_sample() {
...

    {
        ...
        echo "=== [seed=${seed}] Sampling best checkpoint ==="
...
echo "=== All seeds complete. Outputs in $ROOT_DIR/synth_data/reldiff_synth/${DATASET}_ablation/{best,final}/ ==="