...
class Tokenizer(nn.Module):
    ...
    def forward(self, x_num, x_cat):
        x_some = x_num if x_cat.shape[1] == 0 else x_cat
        ...
        if x_cat is not None and x_cat.shape[1]:
            for start, end in zip(
                ...
                x = torch.cat(
                    [
                        x,
                        x_cat[:, start:end].unsqueeze(1)
                        @ self.cat_weight[start:end][None],
                    ],
                    dim=1,
                )
        if self.bias is not None:
...
class Reconstructor(nn.Module):
    def __init__(
        self, d_numerical, categories, d_token, zero_init=False, bias_init=None
    ):
