...
class Trainer:
    def __init__(
        ...
        **kwargs,
        ...
        self.start_epoch = self.curr_epoch

    def _anneal_lr(self, step):
        remaining = self.steps - self.start_epoch
        frac_done = (step - self.start_epoch) / remaining if remaining > 0 else 1.0
        ...
        **kwargs,
        ...
        if self.ckpt_path is not None:
            ...
            if "denoise_fn" in state_dicts:
                # Nested format saved by base Trainer
                self.diffusion._denoise_fn.load_state_dict(state_dicts["denoise_fn"])
                self.diffusion.num_schedule.load_state_dict(state_dicts["num_schedule"])
                self.diffusion.cat_schedule.load_state_dict(state_dicts["cat_schedule"])
            else:
                # Flat diffusion.state_dict() format saved by MultiTableTrainer
